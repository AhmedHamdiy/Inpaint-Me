...
class Inpaint:
    ...
    def calculate_matrices(self):
        ...
        self.grey_img = cv.cvtColor(self.img, cv.COLOR_BGR2GRAY)
        self.gradientX, self.gradientY = np.gradient(self.grey_img)
...
if __name__ == "__main__":
    ...
    mask = generate_rect_mask(img, (390, 350+35), (550, 450+35))
    
    inpaint = Inpaint(img, mask, 11)
    result = inpaint.inpaint(10000)

    result = cv.cvtColor(result, cv.COLOR_RGB2BGR)

    cv.imwrite("results/2.jpg", result)
    cv.imwrite("results/2_mask.jpg", mask * 255)
